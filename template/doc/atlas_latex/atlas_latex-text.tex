...
\item[\Option{hepdatatext=true|{\normalfont false}}] print text about HEPData availability after acknowledgements;
\item[\Option{hepdataexist=true|{\normalfont false}}] HEPData are (or will be) available;
\item[\Option{athenatext=true|{\normalfont false}}] print text about Athena Open Source after acknowledgements;
...
\ADOCnew{15.10.0} Papers should add the macro \Macro{printatlasdatapolicy} after the acknowledgements,
...
Use the options \Option{hepdatatext} and/or \Option{athenatext} to steer whether the text is printed.
Use the option \Option{hepdataexist} to adjust the text depending on whether an HEPData entry exists (or will exist) or not.
Use the macro \Macro{HEPDataRecord} to set the URL for the HEPData.
...
The macro \Macro{HEPDataRecord} is used for the URL for the HEPData.
...
\item It also contains a new column specifier for tables \enquote{S},
  ...
  The specifier \enquote{s} for units was dropped in version 3 of \Package{siunitx}.}
\end{itemize}
