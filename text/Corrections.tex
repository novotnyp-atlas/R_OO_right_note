...
% in pp, then one will determine it also in OO
\blue{} The trigger efficiency for the primary MB trigger for \OO and \NeNe was not yet determined. \black{}
...
{\red NOT NEEDED ANYMORE?}

{\red Will be determined by comparing full-sim Angantyr MCs with $\pprefSample$ and before-$\OO$ conditions. }

\section{Correction of $\avgTAA$ {\red remove?}}
Although the \TAA needed for \RAA is extracted from the Glauber model, one shall take into account that the MinBias sample is not fully inclusive, and some losses are anticipated for events with low multiplicity.
...
\section{Correction for transmutation {\red remove or change?}}\label{sec:transmutation_correction}
...
\begin{figure}[h]
    \centering    
    \includegraphics[width=0.7\linewidth]{images/mixing_scan_all_observables.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.6\linewidth]{images/eta_comparison.png}
...
A mild increase of $\tpv$ is seen with increasing $\mu$ (when $\mu \sim 4$), which is attributed to the effect of vertex merging, since a merged vertex must have more tracks, thus moving the mean. 
...
%The reconstruction algorithm is optimized for the vertex building in $pp$ collisions on higher $\mu$ than heavy-ion collisions are performed. As a result, the algorithm splits a vertex into two. One can see this effect in Figure~\ref{fig:split_vertices}, as despite $\mu$ close to 0, the average number of vertices is 1.5. One could remove the additional fake-split vertex using a cut on the $\lukesvar$. Afterwards, only events with one vertex will be used. S

%\begin{figure}[h]
%    \centering
%     \includegraphics[width=0.7\linewidth]{images/n_vert_OO.png}
%     \caption{Profile of the number of vertices as a function of $\mu$ in run 501850 of the \OO run.}
%     \label{fig:split_vertices}
% \end{figure}

\section{Track momentum resolution (TMR)} 
{\red (the section can be moved into appendix?) Will be determined by taking a ratio between smeared $\pprefSample$ and \OO spectra.} \black{} The input for the smearing is taken from the track momentum resolution simulation \cite{chargedHadronATLAS} shown in Figure~\ref{fig:tmr}.
\begin{figure}[h]
    \centering
     \includegraphics[width=0.7\linewidth]{images/tmr.png}
     \caption{Track momentum resolution simulation, taken from \cite{chargedHadronATLAS}}
    \label{fig:tmr}
\end{figure}
The curves are parametrized as
\begin{equation}
    \text{TMR}(\pT) = p_0 + 0.001 \cdot p_1  \cdot  \pT
\end{equation}
and for each $\eta$ range parameters $p_0$ and $p_1$ are extracted from Figure~\ref{fig:const_tmr_smearing} (see Table~\ref{tab:tmr_parameters}).
\begin{table}
    \centering
    \caption{TMR parameters}
    \label{tab:tmr_parameters}
    \begin{tabular}{c|c|c}
        $\eta$ range & $p_0$ & $p_1$ [$\gev^{-1}$] \\ \hline
        $0.0<|\eta|<0.5$ & 0.01 & 0.34 \\ 
        $1.0<|\eta|<1.3$ & 0.02 & 0.36\\ 
        $1.6<|\eta|<1.8$ & 0.03 & 0.35 \\
        $2.3<|\eta|<2.4$ & 0.05 & 0.57 \\
    \end{tabular}
\end{table}

To estimate the effect of TMR on the final result the following study was conducted:
\begin{enumerate}
    \item Extract $\pT$ and $\eta$ distribution of tracks.
    \item Sample from both distributions and based on $\eta$ bin apply smearing of $\pT$ value:
    \begin{equation}\label{eq:ptsmeared}
        \pT^\text{smeared} \sim \frac{1}{\mathcal{N}\left(\frac{1}{\pT}, \frac{\text{TMR}(\pT, \eta)}{\pT}\right)} ,
    \end{equation}
    where $\mathcal{N}$ is normal distribution. Bins in $\eta$ are chosen in intervals of $|\eta|\in[0.0, 0.5, 1.3, 1.8, 2.5]$, where each bin corresponds to a curve in Figure~\ref{fig:tmr}. Bins are taken to cover the full fiducial space of the tracks. The effect of smearing the "$\delta$-function" with constant $\text{TMR}=0.1$ in Equation~\eqref{eq:ptsmeared} is demonstrated in Figure~\ref{fig:const_tmr_smearing}.
    \begin{figure}
        \centering
        \includegraphics[width=0.45\linewidth]{images/test_smearing.eps}
        \hskip5pt
        \includegraphics[width=0.45\linewidth]{images/smeared_spectra.eps}
        \caption{Left: smearing of delta-function with constant TMR. Right: Smearing of $\pprefSample$ MB spectrum and mock-up spectrum}
        \label{fig:const_tmr_smearing}
    \end{figure}
    \item Apply the same smearing procedure to the mock-up spectrum. As a proxy for the mock-up spectrum, $\pprefSample$ MB spectrum is multiplied by the $\RAA$ of 0-5\% \PbPb. The $\RAA$ of 0-5\% \PbPb was taken from \cite{chargedHadronATLAS} and it represents the strongest modification to the charged hadron spectrum measured by ATLAS. 
\end{enumerate}
\green{}
The resulting double-ratio of original/mock-up over original-smeared/mock-up-smeared is presented in Figure~\ref{fig:double_ratio_tmr}. From this figure one can see about 2\% effect in the range, where the modification applied to spectrum is most prominent ($2\div \qty{30}{\gev}$, see Figure~\ref{fig:const_tmr_smearing}). 
\begin{figure}
    \centering
    \includegraphics[width=0.6\linewidth]{images/double_ratio.eps}
    \caption{The double-ratio of original/mock-up over original-smeared/mock-up-smeared spectra}
    \label{fig:double_ratio_tmr}
\end{figure}
\black{}
