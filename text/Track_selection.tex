...
The track selection begins with the quality selection. Both `Loose' and `TightPrimary' quality (see Table~\ref{tab:tight_primary_quality}, for more information see \cite{tightprimaryQuality_Twiki}) of tracks is subsequently required.
\begin{table}[h]
    \centering
...
An additional selection criterion on tracks is a cut on the transverse impact parameter $d_0$ with respect to the vertex: 
\begin{equation}
    |d_0| < f_{d_0}(\pt) =  0.726458\exp(-0.886906\pt)+0.173646\exp(0.00119139\pt),
\end{equation}
where the function $f_{d_0}(\pt)$ shown in the right panel of Figure~\ref{fig:d0ptcut} is obtained by taking $4\sigma$ from gaussian fit to \pt slices (see Figure~\ref{fig:d0ptcut}, left) in \JetSample sample of \pprefSample (to have enough statistics at high-\pt).
\begin{figure}
    \centering
    \includegraphics[width=0.48\linewidth]{images/d0_slice3.png}
    \includegraphics[width=0.48\linewidth]{images/d0pt_cut.pdf}
    \caption{Left: distribution of $d_0$ for $3<\pt<5$ GeV. Right: $d_0$ as a function of \pt with black line showing $f_{d_0}(\pt)$. Tracks pass `TightPrimary' quality selection}
    \label{fig:d0ptcut}
\end{figure}
To estimate the systematic uncertainty arising from this choice, the curve is multiplied by 1.25 and 0.75 to get $5\sigma$ and $3\sigma$ of the fit of the slices respectively.


% Performance of such selection can be seen in Figure~\ref{fig:cutflow_d0}
% \begin{figure}[h]
%     \centering
...
% \end{equation}
This cut depends on $\mu$ as the number of merged vertices increases with $\mu$ \cite{ATLAS:2016nnj}. If two vertices merge into one, $z$ of both of them moves towards the center of mass of the pair. Consequently, the associated tracks will have greater $\omega_0$, which can be seen on the right plot of Figure~\ref{fig:lowmu_highmu_ip_norm}. The effect of merging in a perpendicular plane to the beamline is not very strong (see Figure~\ref{fig:lowmu_highmu_ip_norm}, left),
\begin{figure}[h]
    \centering
    \includegraphics[width=0.49\linewidth]{images/trk_d0_488573.png}
    \includegraphics[width=0.49\linewidth]{images/trk_omega0_488573.png}
    \caption{Comparison of track distributions for low-$\mu$ and high-$\mu$ $\pprefSample$ data samples. Left: $d_0$ distributions. Right: $\omega_0$ distributions}
    \label{fig:lowmu_highmu_ip_norm}
\end{figure}
because the `effective area' is much smaller. Given that the mean $\mu$ in \OO is much smaller ($\langle\mu\rangle\sim 0.3$) than in \pprefSample ($\langle\mu\rangle\sim 4.0$), the effect of merging is less significant. To estimate the effect of this difference, the $\omega_0$ cut is varied to $|\omega_0|<5.0$ mm and $|\omega_0| < 0.75$ mm.
...
\label{chap:TTJM}
High-\pT tracks mostly originate in jets. Therefore, one should employ track-to-jet matching. Using a distance between a track $i$ and a jet $j$ in $\eta-\phi$ plane:
\begin{equation}
    \Delta R_{ij} = \sqrt{(\eta^i - \eta^j)^2 + (\phi^i-\phi^j)^2}, \quad \min_j \Delta R_{ij} = dR_i,
\end{equation}
the track $i$ is matched to a jet if $dR_i < 0.4$. Jet collection used is \textit{AntiKt4HIJets}. Jets in both $\pprefSample$ and $\OO$ data must have $\pT$ > 30 GeV. Using jets above this kinematic cut ensures the full jet reconstruction efficiency in both the $\pprefSample$ and $\OO$ samples, based on the jet performance studies \cite{riccs_note}. Tracks with $\pt>30$ GeV are required to have $dR < 0.4$.

Subsequently, tracks that fall within $dR < 0.4$ of a jet should not have $\pt^\text{trk}>\pt^\text{jet}$:
\begin{equation}
    \pt^\text{trk} < \pt^\text{jet} \left(1 + 3 \sqrt{(\text{TMR}(\pt^\text{trk}, \eta^\text{trk}))^2 + (\text{JER}(\pt^\text{jet},\eta^\text{jet}))^2} \right),
\end{equation}
where TMR is track momentum resolution obtained in MC samples, JER is jet energy resolution and was obtained from the jet group (see Appendix).
{\red include TMR and JER in pp and OO (possibly in appendix?)}

% Distance $dR$ as a function of $\pT$ is presented on the Figure~\ref{fig:cutflow_mindr}.
% \begin{figure}[h]
%     \centering
...
{\red show correlations of electrons with tracks, then muons }
...
\begin{table}[ht!]
  ...
  \begin{tabular}{ll}
    ...
    Track-to-vertex matching criteria in $d_0$     & $|d_0| < f_{d_0}(\pt)$ \\
    Track-to-vertex matching criteria in $\omega_0$     & $|\omega_0| < \qty[parse-numbers=false]{3.0}{\mm}$\\
    Associated vertex                & $|z_\text{vtx}| < \qty{150}{\mm}$ \\
    Track-to-jet association        & \(\mathrm{d}R < 0.4\) for $\pT^\text{trk}>\qty{30}{\GeV}$ and $\pt^\text{trk}\lesssim\pt^\text{jet}$ \\
    \bottomrule
    \label{tab:tab_cuts}
  \end{tabular}
  % }
\end{table}
The cutflow as a function of \pt is presented in Figure~\ref{fig:cutflow_pt_pp_oo}
\begin{figure}
    \centering
    \includegraphics[width=0.45\linewidth]{images/cutflow_ratios_pp.png}
    \includegraphics[width=0.45\linewidth]{images/cutflow_ratios_oo.png}
    \caption{Ratios of subsequent selections with respect to `Loose' quality for \pprefSample (left) and \OO (right)}
    \label{fig:cutflow_pt_pp_oo}
\end{figure}
...
% The resulting raw spectrum is presented in Figure~\ref {fig:trk_pt_cutflow}.
% \begin{figure}[h]
%     \centering
